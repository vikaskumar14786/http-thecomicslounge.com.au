...
class FlatModLatestView extends DefaultModLatestView
{
	...
	function displayLatestEvents(){
		...
		if(isset($this->eventsByRelDay) && count($this->eventsByRelDay)){
			...
			$content .= $this->modparams->get("modlatest_templatetop") ? $this->modparams->get("modlatest_templatetop") : '<table class="mod_events_latest_table jevbootstrap" width="100%" border="0" cellspacing="0" cellpadding="0" align="center"><tr>';
			...
			foreach($this->eventsByRelDay as $relDay => $daysEvents){
				...
				foreach($daysEvents as $dayEvent){
					...
					if($firstTime) $eventrow = '<td class="mod_events_latest_first" style="'.$dst.'">%s'."</td>";
					else $eventrow = '<td class="mod_events_latest" style="'.$dst.'">%s'."</td>";
			...
			$content .=$this->modparams->get("modlatest_templatebottom") ? $this->modparams->get("modlatest_templatebottom") : "</tr></table>\n";
		}
		else if ($this->modparams->get("modlatest_NoEvents", 1)){
			$content .= $this->modparams->get("modlatest_templatetop") ? $this->modparams->get("modlatest_templatetop") : '<table class="mod_events_latest_table jevbootstrap" width="100%" border="0" cellspacing="0" cellpadding="0" align="center"></tr>';
			$templaterow = $this->modparams->get("modlatest_templaterow") ? $this->modparams->get("modlatest_templaterow")  : '<td class="mod_events_latest_noevents">%s</td>' . "\n";
