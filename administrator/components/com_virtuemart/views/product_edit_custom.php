...
jQuery("#custom_field  .datepicker").each(function() {
   var  str = jQuery(this).attr('id');
	...
	customfieldnumber[i] = matches[0]; 
    i++;	

});	  
var j;
for(j=0;j<=customfieldnumber.length;j++)
{
	alert(customfieldnumber[j]);
	
}
	
jQuery("#field_4_customvalue_text").blur(function() {
    
	var enddate = jQuery(this).val();
	var startdate = jQuery('#field_5_customvalue_text').val();
	...
	{  
	   	var day = (daydiff(parseDate(startdate), parseDate(enddate)));
		alert(day);
		for(var i=0;i<= day;i++ )
		{
		  jQuery("#custom_dates").append('<input type="text" name=""/>');	
		}	
	}
});
});
