				...
				?>
						...
						if (!empty($stack['events'])) :
							...
							<div class="eventmiddle" itemscope itemtype="http://schema.org/Event">
								...
								$query->where($db->quoteName('product_sku') . ' = '."'".$event->alias."'");
								$db->setQuery($query);
								$results = $db->loadObjectList();
			
								if($results[0]->virtuemart_product_id !='')
								{
									$link ="/index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=".$results[0]->virtuemart_product_id."&virtuemart_category_id=16&Itemid=106";
								}
								else
								{
									$link =$event->href;
