	...
	public function display($tpl = null) {
		...
		if(!empty($category)){
			...
			//if(empty($category->category_layout) or $category->category_layout != 'categories') {
				...
				$db = JFactory::getDBO();
				 $q = "SELECT products.virtuemart_product_id FROM jos_virtuemart_products AS products
LEFT JOIN jos_virtuemart_product_customfields AS customfields ON products.virtuemart_product_id = customfields.virtuemart_product_id LEFT JOIN jos_virtuemart_product_categories AS category ON products.virtuemart_product_id = category.virtuemart_product_id WHERE products.published =1 and category.virtuemart_category_id =".$this->categoryId ." AND customfields.virtuemart_custom_id =76 AND customfields.customfield_value > '".date('Y-m-d')."' ORDER BY customfields.customfield_value ASC";
			  $db->setQuery($q);
		      $ids = $db->loadColumn();
