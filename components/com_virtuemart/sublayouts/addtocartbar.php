	...
	if (($stockhandle == 'disableit' or $stockhandle == 'disableadd') and ($product->product_in_stock - $product->product_ordered) < 1) { ?>
		...
		if (!( VmConfig::get('askprice', true) and empty($tmpPrice) ) ) { ?>
			...
			if(!empty($addtoCartButton)){
			 ...
			 <?php	if( $_REQUEST['productsublayout'] != 'products_horizon'){ ?>
				<?php echo $addtoCartButton ?>
			 <?php }else {  ?>
			 
			 <input type="button" onclick="checkavaialablecoupon(<?php echo $product->virtuemart_product_id ?>)" name="addtocart" class=" button" value="APPLY VOUCHERS" />
