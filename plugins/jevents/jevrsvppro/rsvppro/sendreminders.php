...
if ($maxnotice)
{
	...
	$sql = <<<QUERY
SELECT atc.*,atc.id as at_id, rpt.rp_id as repeatid  , rpt.startrepeat, det.summary, ev.state as published
...
QUERY;
	...
	foreach ($rows as $row)
	{
        // If not published lets get out of here!
...
if ($maxnotice)
{
	...
	$allowreminders = $rsvpparams->get("forceautoremind", 0)  ? "" : "AND atc.allowreminders>0";
	$sql = <<<QUERY
SELECT atc.*,atc.id as at_id, rpt.rp_id as repeatid , rpt.startrepeat, ev.state as published
