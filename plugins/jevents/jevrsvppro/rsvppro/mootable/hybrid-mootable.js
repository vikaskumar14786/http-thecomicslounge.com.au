 ...
 *
 * Modified by Geraint Edwards and converted to jQuery
 *
 * MooTable  takes an existing table as an argument and turns it into a cooler table.
 * MooTables allow headings to be resized as well as sorted.
 */


var MooTable = new Object();
MooTable.initialize = function (el, options) {
	this.element = $(el);
	...
	if (this.tableWidth)
		this.div.setStyle('width', (this.tableWidth + 0) + 'px');
...
MooTable._manageHeight = function () {
...
MooTable._rememberCookies = function () {
...
MooTable._makeTable = function (el) {
...
MooTable._makeTableFromTable = function (t, count) {
	...
	if (thead) {
		...
		if (tr.getElements('th').length > 0) {
			...
			if (t.getElement('tfoot'))
				Element.dispose(t.getElement('tfoot'));
...
MooTable.loadData = function (data, append) {
	if (!this.chk(append)) {
		append = true;
	}
	if (!append) {
...
MooTable._loadData = function (data, index) {
	if (!this.chk(index))
		index = 0;
	var section = 0;
	if (index == 0) {
...
MooTable._createTableFramework = function () {
...
MooTable._addBreak = function () {
	//var cell = new Element('div').inject( this.thead_tr ).addClass('break');
}


MooTable._addHeader = function (value, opts) {
...
MooTable._createRow = function (data) {
	var row = {};
...
MooTable._createCell = function (value, fade) {
	if (!this.chk(fade)) {
		fade = true;
	}
	var cell = {};
...
MooTable._setColumnWidths = function () {
	this._setWidths();
...
MooTable._setHeaderWidth = function () {
	var width = 0;
...
MooTable.toggleColumn = function (col) {
...
MooTable._toggleDisplayOptions = function (ev) {
...
MooTable._monitorDisplayOptions = function (ev) {
	var e = new Event(ev);
...
MooTable._setWidthCookie = function (ele) {
...
MooTable._getWidthCookie = function (ele) {
...
MooTable.rowCompare = function (r1, r2) {
...
MooTable.addEvent = function (type, fn) {
...
MooTable._fireEvent = function (type, args) {
