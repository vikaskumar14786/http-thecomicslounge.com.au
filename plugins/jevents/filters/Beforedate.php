...
// Event repeat enddate fitler
class jevBeforedateFilter extends jevFilter
{
	...
	function __construct($tablename, $filterfield, $isstring=true){
		...
		// how far may days into the future by default
		$this->delta = 90;
	...
	function _createFilter($prefix=""){
		...
		if ($this->_date!="" ){
			...
			if (count($parts)<3) {
				$fulldate = date( 'Y-m-d H:i:s',strtotime("+ $this->delta days"));
	...
	function _createfilterHTML(){
		...
		$filterList=array();
		$filterList["title"]=JText::_("JEV_Ending_Before");
		...
		$script  = "var dateval = jQuery('#". $this->filterType."_fvs1').val();" .
				"if (dateval!=''){ " .
				"	jQuery('#beforedatetask').prop('name', 'task'); "  .
				"	if ('".$this->dateformat."'=='Y-m-d'){ ".
				"		var parts = dateval.split('-');".
				"		jQuery('#enddate').val( parts[0]+'-'+parts[1]+'-'+parts[2]); " .
				"	}".
				"	else if ('".$this->dateformat."'=='d/m/Y'){".
				"		var parts = dateval.split('/');".
				"		jQuery('#enddate').val( parts[2]+'-'+parts[1]+'-'+parts[0]); " .
				"	}".
				"	else if ('".$this->dateformat."'=='d.m.Y'){".
				"		var parts = dateval.split('.');".
				"		jQuery('#enddate').val( parts[2]+'-'+parts[1]+'-'+parts[0]); " .
				...
				"} " .
				"else { " .
				"	jQuery('#beforedatetask').prop('name', 'xxxtask'); " .
				"	jQuery('#enddate').val( ''); " .
				"}";
		ob_start();
		...
		$filterList["html"] .= "<input type='hidden' name='xxxtask' id='beforedatetask' value='range.listevents' />\n";
		$filterList["html"] .= "<input type='hidden' name='enddate' id='enddate' value='".$this->filter_values[1]."' />\n";
