        ...
        if ($this->APIConfig['Request:Method'] != "SOAP")
            if ($this->APIConfig['Request:Format'] == "XML")
                if ($this->APIConfig['Request:Method'] != "RPC")
                    ...
                    $request = EwayParser::Obj2JSONRPC("CreateAccessCode", $request);//$request = EwayParser::Obj2RPCXML("CreateAccessCode", $request);
            else {
                $i = 0;
...
class CreateAccessCodeRequest {
    ...
    function __construct() {
        ...
        $this->Payment = new EwayPaymentToken();
